...
# Version 1.1
#
# with modifications to run non-interactively and accept file path as an argument by
# Jan Boelsche
# June 2018
...
#Do sshd_config functions non-interactively
...
if [ "$1" == "i" ]; then
  ...
  if [ $# -lt 2 ]; then
    echo "Usage: $0 a path/to/sshd_config"
    exit 1
  fi
  SSHD_CONF_LOC="$2"
  if ! [ -v SSH_PORT ] ; then SSH_PORT=3000; fi
  auto_config
else
